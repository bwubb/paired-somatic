...
def std_all(infile,tumor,normal,lib):
    ...
    ### Vardict ###
    if os.path.isfile(f'data/work/{lib}/{tumor}/vardict/loh_germline.twice_filtered.norm.vcf.gz'):
        print(os.path.abspath(f'data/work/{lib}/{tumor}/vardict/loh_germline.twice_filtered.norm.vcf.gz'))
        input=os.path.abspath(f'data/work/{lib}/{tumor}/vardict/loh_germline.twice_filtered.norm.vcf.gz')
        std_vardict(input,tumor,normal,lib)
    else:
        print('Could not locate',os.path.abspath(f'data/work/{lib}/{tumor}/vardict/loh_germline.twice_filtered.norm.vcf.gz'))
    ### VarScan2 ###
    if os.path.isfile(f'data/work/{lib}/{tumor}/varscan2/raw.LOH-Germline.norm.vcf.gz'):
        print(os.path.abspath(f'data/work/{lib}/{tumor}/varscan2/raw.LOH-Germline.norm.vcf.gz'))
        input=os.path.abspath(f'data/work/{lib}/{tumor}/varscan2/raw.LOH-Germline.norm.vcf.gz')
        std_varscan2(input,tumor,normal,lib)
    else:
        print('Could not locate',f'data/work/{lib}/{tumor}/varscan2/raw.LOH-Germline.norm.vcf.gz')
...
def get_Args():
    ...
    #mode loh_germline or somatic? or standardize for both automatically?
    #fix all, different for somatic or loh
    args=p.parse_args()
    return args
