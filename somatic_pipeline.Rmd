---
title: 'Nathanson Lab: Tumor/Normal Paired Somatic Pipeline'
output:
...
DiagrammeR::mermaid("
    graph TB
      start( FASTQ / uBAM ) --> bwa[BWA]
      
      subgraph Alignment
        bwa --> samtools
        samtools --> metrics
        samtools --- bam{aligned.bam}
      end
      ...
      subgraph Variant Discovery
        ...
        bam --> mutect2[Mutect2]
        bam --> manta[Manta]
        bam --> strelka2[Strelka2]
        bam --> vardict[VarDict]
        bam --> varscan2[Varscan2]
      end
      
      subgraph Copy Number
        bam --> seqz[Sequenza]
        bam --> facets[Facets]
        seqz -.- varscan2
      end
      
      subgraph Somatic Variant Filtering
        lancet --> bcftools
        mutect2 --> bcftools
        strelka2 --> bcftools
        vardict --> bcftools
        varscan2 --> bcftools
        manta -->|candidate Indels|bcftools
        manta -.- strelka2
        bcftools --> vlr[Varlociraptor]
      end
      
      subgraph Annotation
        vlr -->|FDR 0.95|vep[VEP]
        manta --> |candidate SV|annotsv[AnnotSV]
        seqz --> annotsv
        facets --> annotsv 
      end
...
## Resources
script for making intervals and bed.
